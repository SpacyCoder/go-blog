...
sudo docker build -t spacycoder/accountservice accountservice/
sudo docker service rm accountservice
sudo docker service create --name=accountservice --replicas=1 --network=my_network -p=6767:6767 spacycoder/accountservice
